...
브라우저에서 PDF를 업로드하고 `PDF이름=실제이름` 형식으로 입력하면 사람별 출장 횟수와 총액을 확인할 수 있습니다. `사무분장표 순서`에는 실제 이름을 표시 순서대로 한 줄에 하나씩 입력합니다. 입력한 이름은 그 순서로 출력되고, 목록에 없는 사람은 PDF 순서로 뒤에 표시됩니다. `엑셀 다운로드` 버튼은 표준 서식의 완성 파일을 내려받습니다.
